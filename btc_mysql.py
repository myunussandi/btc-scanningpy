...
def get_key_ranges_from_db():
    """
    Mengambil semua rentang kunci dari database.
    """
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        sql = "SELECT id, start, end FROM key_ranges"
        cursor.execute(sql)
...
def main():
    ...
    ranges = get_key_ranges_from_db()
    if not ranges:
        print("Tidak ada rentang yang ditemukan di database. Pastikan tabel telah diisi.")
        return
